...
@interface WWPlayingViewController () <AVAudioPlayerDelegate>
@property (nonatomic, strong) WWMusic *playingMusic;
...
@property (nonatomic, weak) WWLrcView *lrcView;
/**
 *  歌手图片
 */
...
@implementation WWPlayingViewController
...
- (void)setupLrcView
{
    WWLrcView *lrcView = [[WWLrcView alloc] init];
...
- (void)show
{
    ...
    self.view.hidden = NO;
    if (self.playingMusic != [WWMusicTool playingMusic]) {
...
- (void)resetPlayingMusic
{
    ...
    //停止播放音乐
    [[WWAudioManager defaultManager] stopMusic:self.playingMusic.filename];
    self.player = nil;
...
- (void)startPlayingMusic
{
    ...
    // 设置所需要的数据
    self.playingMusic = [WWMusicTool playingMusic];
    ...
    //开发播放音乐
    self.player = [[WWAudioManager defaultManager] playingMusic:self.playingMusic.filename];
...
- (IBAction)playOrPause:(id)sender {
    if (self.playOrPauseButton.isSelected == NO) {
        self.playOrPauseButton.selected = YES;
        [[WWAudioManager defaultManager] playingMusic:self.playingMusic.filename];
        [self addCurrentTimer];
        [self addLrcTimer];
    }
    else{
        self.playOrPauseButton.selected = NO;
        [[WWAudioManager defaultManager] pauseMusic:self.playingMusic.filename];
        [self removeCurrentTimer];
        [self removeLrcTimer];
    }
}
...
- (IBAction)previous:(id)sender {
    ...
    [[WWAudioManager defaultManager] stopMusic:self.playingMusic.filename];
    [WWMusicTool setPlayingMusic:[WWMusicTool previousMusic]];
    [self removeCurrentTimer];
    [self removeLrcTimer];
    [self startPlayingMusic];
...
- (IBAction)next:(id)sender {
    ...
    window.userInteractionEnabled = NO;
    [[WWAudioManager defaultManager] stopMusic:self.playingMusic.filename];
    [WWMusicTool setPlayingMusic:[WWMusicTool nextMusic]];
    [self removeCurrentTimer];
    [self removeLrcTimer];
    [self startPlayingMusic];
    window.userInteractionEnabled = YES;
